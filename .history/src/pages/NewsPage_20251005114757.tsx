...
export default function NewsPage() {
  ...
  useEffect(() => {
    // Fetch Google News RSS for "meio ambiente"
    async function fetchGoogleNews() {
      try {
        const rssUrl =
          'https://news.google.com/rss/search?q=meio+ambiente&hl=pt-BR&gl=BR&ceid=BR:pt-419';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          {externalArticles.length === 0 ? (
            <div className="text-slate-500">Carregando notícias externas...</div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {externalArticles.slice(0, 6).map((item, idx) => (
                <a
                  key={idx}
                  href={item.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 block border border-slate-100 hover:border-emerald-200"
                >
                  <div className="p-6">
